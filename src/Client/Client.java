...
public class Client {
    ...
    private void viewServers() {
        ...
        if (serverNames.equals("Empty"))
            return;
        String requestedServer = InputHandler.getString("Enter the numbered index of the server you want to enter: ");
        String temp = serverNames.substring(serverNames.lastIndexOf('[') + 1, serverNames.lastIndexOf(']'));
        int num = Integer.parseInt(temp);
        int choice = Integer.parseInt(requestedServer);
        if (requestedServer.equals("0") || requestedServer.equals("") || choice > num || choice < 1) {
